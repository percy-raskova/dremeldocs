...
def test_filename_generation():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        if len(parts) == 3:
            ...
            print(f"  Parts: seq={seq_part}, date={date_part}, title={title_part[:30]}...")

            # Validate each part
            if seq_part.isdigit() and len(seq_part) == 3:
                print(f"    ✅ Sequence format correct")
            else:
                print(f"    ❌ Sequence format incorrect")

            if date_part.isdigit() and len(date_part) == 8:
                print(f"    ✅ Date format correct (YYYYMMDD)")
            else:
                print(f"    ❌ Date format incorrect")

            if title_part and '_' in title_part:
                print(f"    ✅ Title has underscores")
            elif title_part:
                print(f"    ⚠️  Title exists but no underscores")
        else:
            print(f"  ❌ Could not parse filename parts")
