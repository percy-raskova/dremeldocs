...
try:
    ...
    from text_processing import (
        generate_title, generate_description, calculate_reading_time,
        generate_filename, parse_to_yyyymmdd, format_frontmatter_value
    )
...
"""
        ...
        for filepath in generated_files:
            ...
            assert len(structure_errors) == 0, f"Markdown structure errors in {filepath.name}: {structure_errors}"

            # Validate frontmatter
            frontmatter = extract_frontmatter_from_content(content)
            assert frontmatter is not None, f"Could not extract frontmatter from {filepath.name}"

            frontmatter_errors = validate_frontmatter_structure(frontmatter)
            assert len(frontmatter_errors) == 0, f"Frontmatter errors in {filepath.name}: {frontmatter_errors}"

            # Validate filename format
            assert validate_filename_format(filepath.name), f"Invalid filename format: {filepath.name}"
    ...
    def test_pipeline_with_real_heavy_hitters_sample(self, temp_workspace):
        ...
        filename = generate_filename(1, realistic_thread['first_tweet_date'], realistic_thread['smushed_text'])
        ...
        serious_issues = [issue for issue in quality_issues if 'processing issues' in issue or 'may not be properly' in issue]
        assert len(serious_issues) == 0, f"Serious text quality issues: {serious_issues}"
    ...
    def test_pipeline_output_consistency(self, temp_workspace):
        ...
        for i, run in enumerate(runs[1:], 1):
            assert run['title'] == first_run['title'], f"Title inconsistent in run {i+1}"
            assert run['description'] == first_run['description'], f"Description inconsistent in run {i+1}"
            assert run['reading_time'] == first_run['reading_time'], f"Reading time inconsistent in run {i+1}"
    ...
    def test_pipeline_performance(self, temp_workspace):
        ...
        for thread in large_dataset:
            try:
                ...
                description = generate_description(thread['smushed_text'])
                reading_time = calculate_reading_time(thread['smushed_text'])
                filename = generate_filename(processed_count + 1, thread['first_tweet_date'], thread['smushed_text'])
...
"""
        ...
        assert len(structure_errors) == 0, f"Markdown structure errors: {structure_errors}"