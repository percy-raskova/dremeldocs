...
class CorpusVocabularyExtractor:
    ...
    def extract_from_thread(self, text: str) -> Dict[str, List[str]]:
        ...
        political_triggers = {
            'capitalism', 'socialism', 'communism', 'marxism', 'imperialism',
            'revolution', 'liberation', 'oppression', 'exploitation', 'alienation',
            'praxis', 'hegemony', 'ideology', 'fascism', 'liberalism'
        }
        ...
        for ent in doc.ents:
            if ent.label_ in ['ORG', 'GPE', 'PERSON', 'EVENT']:
                if any(term in ent.text.lower() for term in ['party', 'movement', 'conference', 'revolution']):
    ...
    def identify_key_phrases(self, threads_file: Path, min_df: int = 3) -> Dict[str, List[Tuple[str, float]]]:
        ...
        for thread in data['threads']:
            ...
            if any(term in text for term in ['marx', 'capital', 'class struggle', 'proletariat']):
                category_docs['marxism'].append(text)
            if any(term in text for term in ['fascis', 'liberal decay', 'national consciousness']):
                category_docs['fascism'].append(text)
            if any(term in text for term in ['imperial', 'colonial', 'palestine', 'settler']):
                category_docs['imperialism'].append(text)
            if any(term in text for term in ['covid', 'pandemic', 'public health', 'vaccine']):
                category_docs['public_health'].append(text)
            if any(term in text for term in ['organiz', 'party', 'movement', 'mass work']):
        ...
        for category, docs in category_docs.items():
            if len(docs) >= 3:  # Need minimum docs for TF-IDF
                ...
                try:
                    ...
                    phrase_scores = [(features[i], scores[i]) for i in scores.argsort()[-20:][::-1]]
    ...
    def generate_yaml_vocabulary(self, vocab_counts: Dict, tfidf_phrases: Dict, output_dir: Path):
        ...
        for extract_cat, output_cat in category_mapping.items():
            ...
            print(f"  ✅ Generated {output_file.name} with {len(vocab_data['terms'])} terms")
    ...
    def analyze_corpus_statistics(self, threads_file: Path):
        ...
        print(f"  Words in heavy hitters: {sum(t['word_count'] for t in heavy_hitters):,}")
        ...
        political_terms = sum(1 for token in doc if token.text.lower() in {
            'class', 'capital', 'marx', 'revolution', 'imperialism', 'fascism',
            'liberation', 'oppression', 'dialectical', 'material', 'praxis'
        })
